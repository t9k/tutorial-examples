...
def make_tft_input_metadata(schema):
  ...
  return tft_schema, dataset_schema.schema_utils.schema_from_feature_spec(tft_schema)
...
def run_analysis(output_dir, model_dir, eval_path, schema, slice_columns):
  ...
  slice_spec = [
      tfma.slicer.SingleSliceSpec(),  # An empty spec is required for the 'Overall' slice
  ...
  with beam.Pipeline(runner=runner, options=pipeline_options) as pipeline:
    ...
    example_coder = ExampleProtoCoder(tft.tf_metadata.dataset_metadata.DatasetMetadata(schema_from).schema)
    csv_coder = CsvCoder(column_names, tft.tf_metadata.dataset_metadata.DatasetMetadata(schema_from).schema)
...
#   ]
  slicing_metrics_view = tfma.view.render_slicing_metrics(result, slicing_column=slicing_columns[0])
  static_html_path = os.path.join(output_dir, _OUTPUT_HTML_FILE)
  embed_minimal_html(static_html_path, views=[slicing_metrics_view], title='Slicing Metrics')
