...
def get_feature_columns(transformed_data_dir):
  ...
  Args:
    transformed_data_dir: The directory holding the output of the tft transformation.
  Returns:
    A list of tf.feature_column.
  """
  real_valued_columns = [tf.feature_column.numeric_column(key, shape=()) for key in DENSE_FLOAT_FEATURE_KEYS]

  categorical_columns = [tf.feature_column.categorical_column_with_identity(
...
def write_to_pvc(predict_data, output, predictions):
  with open(predict_data) as f:
    with open(os.path.join(output, 'prediction_results-00000-of-00001'), 'w') as fo:
      i = 0
      for line in f:
        if predictions[i]['probabilities'][0] > 0.5:
          predicted = 'false'
        fo.write(line.strip() + ',' + predicted + ',' + str(predictions[i]['probabilities'][0]) + ',' +str(predictions[i]['probabilities'][1]) + '\n')
        i += 1
...
#   tf.logging.set_verbosity(tf.logging.INFO)
  ...
  categorical_columns, real_valued_columns = get_feature_columns(args.transformed_data_dir)

  estimator = get_estimator(args.transformed_data_dir, args.target, args.output,
                            args.hidden_layer_size, args.learning_rate,
                            categorical_columns, real_valued_columns)
  ...
  eval_input_fn = make_training_input_fn(
      ...
      32,
      args.target)
  serving_input_fn = lambda: make_serving_input_fn(
      args.transformed_data_dir,
      args.target)
  ...
  eval_spec = tf.estimator.EvalSpec(input_fn=eval_input_fn, exporters=[exporter], name='chicago-taxi-eval')
