...
if master_process:
    ...
    ### update hyperparameters of the Run ###
    run.hparams.update(config)

    ### load the Dataset and mark it as input of the Run ###
    ah.login(ais_host=ais_host, ah_host=ah_host, api_key=api_key)
    ah_dataset = ah.get_asset(path='dataset/text/openwebtext')
    ah_dataset.download(save_dir=data_dir)
    run.mark_input(ah_dataset)
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        run.log(type='train', metrics={'loss': losses['train']}, step=iter_num)
        run.log(type='val', metrics={'loss': losses['val']}, step=iter_num)
        if wandb_log:
            wandb.log({
                "iter": iter_num,
...
if master_process:
    ### create the Model and branch, mark it as output of the Run ###
    model = ah.create_folder(kind='Model', name='llm',
                             exist_ok=True).create_asset(name='nanogpt',
                                                         labels=['GPT2'])
    branch = model.get_branch(name='main')
    branch.create_commit(msg='add model ckpt file',
                         add=[os.path.join(out_dir, 'ckpt.pt')])
    run.mark_output(branch)
