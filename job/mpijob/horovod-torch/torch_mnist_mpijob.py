...
parser = argparse.ArgumentParser(
    description='Distributed training of PyTorch model for MNIST using horovod.'
)
...
def train(scheduler):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, (data, target) in enumerate(train_loader, 1):
            ...
            if step % (500 // hvd.size()) == 0:
                train_loss = loss.item()
                train_loss = average_metric(
                    train_loss)  # loss of single training step
                if hvd.rank() == 0:
                    logging.info(
                        'epoch {:d}/{:d}, batch {:5d}/{:d} with loss: {:.4f}'.
                        format(epoch, epochs, step, steps_per_epoch,
                               train_loss))
...
def test(val=False, epoch=None):
    ...
    test_accuracy = correct / len(loader.sampler)
    test_loss = average_metric(test_loss)
    test_accuracy = average_metric(test_accuracy)
    ...
    if hvd.rank() == 0:
        ...
        if args.log_dir:
            writer.add_scalar('{:s}/loss'.format(label), test_loss,
                              global_step)
...
if __name__ == '__main__':
    ...
    val_loader = torch.utils.data.DataLoader(val_dataset,
                                             batch_size=400 // hvd.size(),
                                             sampler=val_sampler,
                                             **kwargs)
    test_loader = torch.utils.data.DataLoader(test_dataset,
                                              batch_size=1000 // hvd.size(),
